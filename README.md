...
* [🖥️ שלב ה': יצירת ממשק גרפי (GUI)](#️-שלב-ה-יצירת-ממשק-גרפי-gui)
  * [בחירת כלים ונימוקים](#-בחירת-כלים-ונימוקים)
  * [ארכיטקטורת הקוד](#-ארכיטקטורת-הקוד)
  * [מסכי המערכת](#-מסכי-המערכת)
  * [דרישות אקדמיות – יישום טכני](#-דרישות-אקדמיות--יישום-טכני)
...
# 🖥️ שלב ה': יצירת ממשק גרפי (GUI)

בשלב זה יצרנו יישום שולחני מלא ומודרני בשפת **Python** המתחבר ישירות למסד הנתונים **Supabase PostgreSQL** שנבנה בשלבים הקודמים. הממשק מאפשר לבצע את מלוא פעולות ה-CRUD, להריץ שאילתות מורכבות ולהפעיל פרוצדורות מאוחסנות — הכל מתוך חוויה גרפית נוחה ומעוצבת.

<hr />

## 🛠️ בחירת כלים ונימוקים

### Python
Python נבחרה כשפת הפיתוח הראשית מסיבות אחדות:
* **הבשלות האקוסיסטם:** ספריות כמו `psycopg2` ו-`pandas` מציעות אינטגרציה מעמיקה עם PostgreSQL ללא הגדרות מורכבות.
* **קריאות הקוד:** תחביר Python נקי ומפורש, מה שמקל על תחזוקה ועל הוספת פונקציונליות עתידית.
* **רב-פלטפורמיות:** הקוד רץ ללא שינוי על Windows, macOS ו-Linux.
* **ניסיון קודם:** Python שימשה כבר בשלב א' ליצירת נתוני הדמו, כך שהמעבר לפיתוח GUI היה טבעי ועקבי.

### CustomTkinter
`customtkinter` נבחרה כספריית ה-GUI מהנימוקים הבאים:
* **עיצוב מודרני:** בניגוד ל-`tkinter` הגולמי, `customtkinter` מספקת ויג'טים בסגנון Material Design עם תמיכה מלאה ב-Dark Mode מחוץ לקופסה.
* **אין תלויות חיצוניות מורכבות:** ניתן להתקינה עם `pip` בלבד, ללא נדרש framework נוסף.
* **התאמה אישית מלאה:** כל ויג'ט תומך בצבעים, גבולות, פונטים ואנימציות מותאמות.
* **תאימות Python:** בניגוד ל-PyQt או wxPython, `customtkinter` לא דורשת הגדרת סביבה נפרדת.

### psycopg2-binary
* מהדר ה-PostgreSQL הסטנדרטי ל-Python, ותיק ומבוסס.
* תומך ב-**Parameterized Queries** (`%s`) באופן מובנה — הגנה מלאה מפני SQL Injection.
* `psycopg2-binary` כוללת את כל הספריות המקומיות בחבילה אחת, ללא צורך בהתקנת PostgreSQL מקומי.

### python-dotenv
* מאפשרת הפרדה מוחלטת של פרטי ההתחברות מהקוד עצמו.
* קריאת קובץ `.env` בצורה מאובטחת — הפרטים לא נחשפים ב-commit לגיט.

<hr />

## 🏗️ ארכיטקטורת הקוד

הפרויקט בנוי בצורה מודולרית בתוך ספרייה `stage5/app/`:

```
stage5/
├── app/
│   ├── main.py                    # נקודת כניסה — חלון ראשי + ניווט בסרגל צד
│   ├── db_connection.py           # שכבת חיבור למסד הנתונים (psycopg2 + dotenv)
│   ├── ui_components.py           # ספריית רכיבי UI משותפים (factory functions)
│   ├── dashboard.py               # מסך דשבורד ראשי עם KPIs ואימונים אחרונים
│   ├── crud_users.py              # CRUD מלא לטבלת core_user
│   ├── crud_workout_log.py        # CRUD מלא לטבלת workout_log
│   ├── crud_lockers.py            # CRUD מלא לטבלת locker
│   ├── crud_body_measurement.py   # CRUD מלא לטבלת body_measurement
...
* **הפרדת שכבות:** שכבת ה-DB (`db_connection.py`), שכבת ה-UI (`ui_components.py`) ושכבת הלוגיקה (קבצי המסכים) מופרדות לגמרי.
* **Factory Functions:** `ui_components.py` מכיל פונקציות יצירה המבטיחות עיצוב אחיד בכל המסכים.
* **Screen Registry:** `main.py` מנהל מילון של מסכים המאפשר ניווט דינמי ללא קישורים קשיחים.
...
### 1. דשבורד ראשי
מסך הפתיחה המציג 4 כרטיסי KPI חיים, ניווט מהיר לכל מסכי המערכת, ותצוגת האימונים האחרונים עם שמות מתאמנים ותוכניות מחוברים ב-JOIN.

### 2. ניהול מתאמנים (`core_user`)
CRUD מלא עם חיפוש חי לפי שם ותהליך עדכון דו-שלבי: בחירת רשומה ← מילוי אוטומטי ← עריכה ← שמירה.

### 3. יומן אימונים (`workout_log`)
CRUD מלא עם Dropdowns חכמים המציגים שמות מתאמנים ותוכניות בפורמט `ID – שם` — JOIN אוטומטי בשליפה.

### 4. ניהול לוקרים (`locker`)
CRUD מלא עם תמיכה בלוקרים פנויים ותפוסים. אכיפת האילוץ `chk_locker_rental` בממשק לפני שליחה ל-DB.

### 5. מדידות גוף (`body_measurement`)
CRUD מלא לכל 9 שדות המדידה עם Dropdown לבחירת מתאמן ו-JOIN לשם מלא.

### 6. מנויים (`subscription`)
CRUD מלא לחוזי מנויים עם פאנל סטטיסטיקות המציג כמות, עלות ממוצעת וסך הכנסות.

### 7. פעולות מתקדמות
המסך מחולק ל-3 סעיפים:

**שאילתות מורכבות (שלב ב'):**
* **שאילתה 1:** אנליזה חודשית של שריפת קלוריות ודופק — `GROUP BY TO_CHAR(log_date, 'YYYY-MM')` עם סינון לפי שנה.
* **שאילתה 2:** חשיפה לתביעות — JOIN משולש `core_user ⟷ locker ⟷ health_declaration` לאיתור הצהרות בריאות שפגו.

**פונקציות מאוחסנות (שלב ד'):**
* **פונקציה 1:** `fn_user_workout_stats(user_id)` — מחזירה RETURNS TABLE עם 7 שדות סטטיסטיקה (COUNT, SUM, AVG, MAX). מקבלת User ID ומציגה את הנתונים בטבלה בתוך המסך.
* **פונקציה 2:** `fn_trainee_progress_cursor(user_id)` — מחזירה Ref Cursor עם היסטוריית מדידות גוף כולל חישוב `LAG()` לשינוי משקל בין מדידות. הפונקציה נקראת בתוך Transaction פתוח (BEGIN/COMMIT) וממשק מציג גם NOTICE messages בחלון קופץ.

**פרוצדורות מאוחסנות (שלב ד'):**
* **פרוצדורה 1:** `proc_award_achieved_goals()` — מסמנת יעדי מתאמנים שהושגו (`is_achieved=1`). תוצאות מוצגות בחלון קופץ עם פלט NOTICE מלא.
* **פרוצדורה 2:** `proc_extend_expiring_lockers()` — מאריכה לוקרים שעומדים לפוג. תוצאות מוצגות בחלון קופץ עם פלט NOTICE מלא.

<hr />

## 📜 דרישות אקדמיות – יישום טכני

### ✅ ללא מזהי FK גולמיים בממשק
כל ה-Dropdowns מציגים ערכים בפורמט `ID – שם`. ה-ID מחולץ תכנותית ממילון מיפוי (`_user_map`) לפני שליחה ל-DB. כל grid שליפה משתמש ב-JOIN לשמות — אין חשיפת מפתחות זרים גולמיים.

```python
# בניית מיפוי label → ID
self._user_map = {f"{uid} – {name}": uid for uid, name in users}

# חילוץ ה-ID בעת שמירה
uid = self._user_map.get(self._user_combo.get())
```

### ✅ תהליך עדכון דו-שלבי
כל מסך CRUD מיישם: לחיצה על שורה → `SELECT` לפי PK → מילוי אוטומטי של כל השדות → עריכה → לחיצה על "שמור עדכון".

```python
def _on_select(self, event):
    """Two-step update: selecting a row populates the form."""
    values = self._tree.item(selected[0], "values")
    self._pk_var.set(values[0])  # שמירת ה-PK
    # שליפת הרשומה המלאה ואכלוס השדות...
```

### ✅ Parameterized Queries בלבד
כל שאילתה במערכת משתמשת ב-placeholders (`%s`) — ללא שרשור מחרוזות.

```python
cur.execute(
    "SELECT * FROM public.core_user WHERE user_id = %s",
    (int(pk),)   # ← הפרמטר מוזרק בבטחה על-ידי psycopg2
)
```
...
### שלב 1: התקנת תלויות
פתח **Terminal / PowerShell** בספריית `stage5/` והרץ:
```bash
pip install -r requirements.txt
```

### שלב 2: בדיקת קובץ `.env`
ודא שקובץ `.env` בשורש תיקיית `Stage5_Workspace` מכיל את פרטי החיבור הנכונים (DB_HOST, DB_NAME, DB_USER, DB_PASSWORD, DB_PORT).
...
> **הערה:** יש להריץ את `python main.py` מתוך ספריית `stage5/app/` כדי שנתיב קובץ ה-`.env` יתגלה כראוי.

### דרישות מינימום
* Python 3.9 ומעלה.
* חיבור אינטרנט פעיל (לחיבור Supabase).
